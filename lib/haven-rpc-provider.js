...
const allowedMethods = ['get_height', 'get_balance', 'restore_deterministic_wallet', 'query_key', 'transfer', 'get_transfers', 'create_wallet'];
...
class HavenRPCProvider {
    ...
    async _processRequest(objRequest, cb) {
        ...
        if (!allowedMethods.some(method => method === objRequest.method)) {
            ...
            throw {status:"error", error:"method not allowed"};
        }
