...
function seemToBeInIE(): boolean {
  return typeof ClipboardEvent === "undefined" &&
         typeof (window as unknown as IEWindow).clipboardData !== "undefined" &&
         typeof (window as unknown as IEWindow).clipboardData.setData !== "undefined";
}
...
function writeIE(data: DT): boolean {
  ...
  const text = data.getData(TEXT_PLAIN);
  if (text !== undefined) {
    return (window as unknown as IEWindow).clipboardData.setData("Text", text);
  }
...
async function readIE(): Promise<string> {
  const text = (window as unknown as IEWindow).clipboardData.getData("Text");
  if (text === "") {
