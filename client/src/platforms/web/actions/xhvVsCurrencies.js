...
import {XHV_VS_CURRENCIES_FAILED, XHV_VS_CURRENCIES_SUCCEED} from "../actions/types";
    ...
    return (dispatch) => {
        ...
        currencies.forEach( (currency) => {
            ...
            if (prices.length > 0 ) {
                ...
                const payload = {[currency.ticker]: {
                        prices, change, token:currency.token, lastPrice,
...
const xhvVsCurrenciesSucceed = (result, currency) => {
    ...
    const payload = {[currency.ticker]:{
        prices, change,lastPrice, token:currency.token,symbol:currency.symbol
        }};
