...
export const offshoreProcess = (state = INITIAL_STATE, action: AnyAction):ExchangeProcessInfo => {
    switch (action.type) {
        ...
        case OFFSHORE_SUCCEED:
            return {...state, isFetching: false, amount:action.payload.amount, succeed: true, fee:action.payload.fee};
        case ONSHORE_FAILED:
        case OFFSHORE_FAILED:
            return {...state, isFetching:false, succeed: false, error: action.payload};
        default:
            return state;
    }
};
