import {
    ...
    RESTORE_WALLET_BY_SEED_SUCCEED, UPDATE_SAVED_WALLETS
} from "../actions/types";
...
export const walletSession =  function(state = INITIAL_STATE, action: AnyAction): WalletSession {
    switch (action.type) {
        ...
        case RESTORE_WALLET_BY_SEED_FAILED:
            return {...state, error: action.payload, activeWallet:null, isFetching:false, isWalletOpen:false};
        case OPEN_WALLET_SUCCEED:
            return {...state,error:null, isFetching: false, activeWallet:action.payload, isWalletOpen:true};
        case RESTORE_WALLET_BY_SEED_SUCCEED:
            return {error:null, isFetching: false, activeWallet:action.payload, isWalletOpen:true,
                savedWallets:state.savedWallets?[...state.savedWallets, action.payload]:[action.payload] };
