...
enum CREATION_STEPS  {
  ...
  Info,Credentials,Seed,Verification
 ...
 class CreateDesktopContainer extends Component<CreateDesktopProps, CreateDesktopState> {
  ...
  componentDidUpdate(prevProps: Readonly<CreateDesktopProps>, prevState: Readonly<CreateDesktopState>, snapshot?: any): void {


    if (prevProps.walletCreation.isCreated === false &&
  ...
  prevStep = () => {
    ...
    if(this.state.step === CREATION_STEPS.Info || this.state.step === CREATION_STEPS.Seed)
      return;
    ...
    switch (step) {
      ...
      case CREATION_STEPS.Verification:
        return (
            ...
            <>
              <Description
                  label={`type in words ${this.state.wordsToVerify.map(word => word.index + 1).join(', ')}`}
                  placeholder={`type in words ${this.state.wordsToVerify.map(word => word.word ).join(', ')}`}
                  name={'verify_seed'}
                  value={verify_seed}
                  error={error}
                  onChange={this.onChangeHandler}
    ...
    return (
        ...
        <Buttons>
          ...
          <Submit
            ...
            >
            {this.props.walletCreation.isFetching? <Spinner color={"white"} /> : 'Continue' }
