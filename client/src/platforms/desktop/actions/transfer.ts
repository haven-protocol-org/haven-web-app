...
export const createTransfer = (
  ...
  return (dispatch: any) => {
    dispatch(transferCreationFetch({ address, fromAmount, paymentId, fromTicker }));
    const params: any = createTXInputs(address, amount, paymentId);

    const transferFN = fromTicker === Ticker.XHV? transferRPC: offshoreTransferRPC;
      ...
      .then(result => {
          ...
          const reduxParams = {fee: convertBalanceForReading(fee), fromAmount: convertBalanceForReading(amount), metaData: tx_metadata};
