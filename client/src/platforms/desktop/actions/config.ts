...
import { BasicNode, NodeLocation, SelectedNode } from "../types";
...
export const setDesktopConfig = () => {
  return async (dispatch: any, getState:() => DesktopAppState) => {
    ...
    const netSpecificNodes: BasicNode[] = remoteNodes[getNetworkByName()]
    netSpecificNodes.forEach(node => node.location = NodeLocation.Remote);

    dispatch({ type: SET_NODE_LIST_FOR_WALLET_SUCCESS, payload: netSpecificNodes });
    ...
    if (config && config.selectedNode) {
      ...
      if (selected.location !== NodeLocation.None) {
          dispatch(setNodeForWalletSucceed(selected.address!, selected.port!, selected.location!));
      }
      // if there is no last selected node we set the defaulzt remote one
      else {

        const defaultNode = selectRemoteDefaultNode(getState());
        dispatch(setNodeForWalletSucceed(defaultNode.address!, defaultNode.port!, defaultNode.location))
