...
class Balances extends Component<BalanceProps, BalanceState> {
    ...
    onClickNext() {
        ...
        this.setState({currentIndex:nextIndex, currentTicker:Object.keys(Ticker)[nextIndex] as Ticker});
  ...
  render() {
    ...
    return (
      <Wrapper onClick={() => this.onClickNext()}>
        ...
        {lockedBalance > 0 ? (
          <Pending>
            You have {convertBalanceForReading(lockedBalance) + ' ' + ticker } pending.
            <br />
...
export const mapStateToProps = (state: DesktopAppState | WebAppState ) => ({
  balances:state.xBalance,
    //@ts-ignore
    syncState: isDesktop() ? selectDesktopSyncState (state): selectWebSyncState(state)
});
